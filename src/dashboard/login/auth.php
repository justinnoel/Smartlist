...
$db = new PDO( "mysql:host=" . App::server . ";dbname=" . App::database . ";charset=utf8mb4", App::username, App::password );
$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$db->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);

$_username = isset($_POST["username"]) ? trim($_POST["username"]) : "";
$_password = trim(password_hash($_POST["password"], PASSWORD_ARGON2I));
if (isset($_SESSION["valid"])) {
    ...
    foreach ($d as $r) {
        if (isset($_POST["app"]) && hash("sha512", $r["id"]) == $_POST["app"]) {
            ...
            try {
                $conn = new PDO( "mysql:host=".App::server.";dbname=smartlis_api", App::username, App::password );
                // set the PDO error mode to exception
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                // prepare sql  and bind parameters
                $stmt = $conn->prepare( $sql = "INSERT INTO tokens (name, email, username, user_avatar, app, userID, color) VALUES (:name, :email, :username, :avatar, :token, :sessid, :color)" );
            ...
            die($r["redirectURI"] . "__AUTH__|" . $token);
        }
    }
}
if (empty($_password) || empty($_password)) {die("Empty");}
    try {
        ...
        foreach($users as $user) {
          if(strtolower($user['username']) == strtolower($_username) || strtolower(decrypt($user['email'])) == strtolower($_username)) {
            $invalidLogin = false;
            $row = $user;
          }
        }
        ...
        if ($invalidLogin) {
          ...
          die("Invalid");
            ...
            if (password_verify($_POST["password"], $row["password"])) {
                ...
                $users = $dbh->query("SELECT * FROM auth_ip WHERE ip='" . md5($city) . "' AND login=" . intval($row["id"]));
                if ($users->rowCount() >= 1) {
                    if ($row["accept"] === 1) {
                        $_SESSION["valid"] = true;
                        ...
                        $_SESSION["syncid"] = (empty($row["syncid"]) ? -1:$row["syncid"]);
                        $_SESSION['avatar'] = (empty($row['user_avatar']) ? "https://i.stack.imgur.com/34AD2.jpg" : $row["user_avatar"]);
                        $_SESSION["welcome"] = intval($row["welcome"]);
                        $_SESSION["theme"] = htmlspecialchars($row["theme"]);
                        if ($row["welcome"] !== "1") {die("Welcome");}
                            setcookie("attempts", 0, time() + 86400 * 30, "/");
                            if (!isset($_POST["app"])) {
                                echo "Valid";
                            } else {
                                $conn1 = new PDO( "mysql:host=".App::server.";dbname=smartlis_api", App::username, App::password );
                                $conn1->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
                                $stmt1 = $conn1->prepare("SELECT * FROM apps");
                                $stmt1->execute();
                                $result1 = $stmt1->setFetchMode( PDO::FETCH_ASSOC );
                                          ...
                                          <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0"
                                            ...
                                            <tr>
                                              <td style="padding:0 35px;">
                                                ...
                                                <a href="https://smartlist.ga/dashboard/user/verify-location.php?u=' .
                                                      md5($city) .
                                                      '"
