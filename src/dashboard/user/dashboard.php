...
<style>
  ...
  ._darkTheme .card center img {display: none}
</style>
<div class="material-sidenav:list sidenav" id="sidenav__shoppingList" style="padding: 20px!important;">
  ...
  <p style="line-height: 30px">
    <label>
      <input type="checkbox" oninput="if(confirm('Delete?')===true){this.disabled=true;loadURL('./user/grocerylist/delete.php?id=<?=$item->id;?>','GET',function(){ setTimeout(function() {getHashPage('hide')}, 200) })}else{this.checked=false}"/>
      <span><?=$item->name;?></span>
    </label>
  </p>
  <?php } ?>
...
<?php foreach ($_DATA->listNames as $list) {?>
  ...
  <button onclick="if(confirm('Delete') === true) loadURL('./user/lists/deleteList.php?id=<?=$list->id;?>', 'GET', getHashPage)" class="sidenav-close btn btn-floating btn-flat right z-depth-0 waves-effect darken-4" style="margin-right: 5px!important"><i class="material-icons black-text">delete</i></button>
  ...
  <?php foreach (array_slice($_DATA->listItems, 0, 10, true) as $item1) { if($item1->listId == $list->id) {?>
          <p style="line-height: 30px">
          <label>
            <input type="checkbox" oninput="if(confirm('Delete?')===true){this.disabled=true;loadURL('./user/lists/deleteItem.php?id=<?=$item1->id;?>','GET',function(){ setTimeout(function() {getHashPage('hide')}, 200) })} else{this.checked=false}"/>
...
<div style="padding: 10px;">
  <div class="row">
    ...
    <div class="col s12 m6">
      ...
      <div tabindex="0" class="focus-outline <?php if(count($_DATA->shoppingList) !== 0) {?>waves-effect<?php } ?>" style="width:100%;padding: 25px;padding-bottom:0!important;padding-left: 30px!important;padding-right:20px!important" <?php if(count($_DATA->shoppingList) !== 0) {?>onclick="$('#sidenav__shoppingList').sidenav('open')"<?php } ?>><?php if(count($_DATA->shoppingList) !== 0) {?><i class="material-icons right" style="line-height: 40px;">chevron_right</i><?php } ?><h5 style="margin-top: 3px"><b>Shopping list</b></h5></div>
    ...
    <div class="col s12 m6">
    <div class="card overflow:hidden">
    <div tabindex="0" class="focus-outline <?php if(count($_DATA->reminders) !== 0) {?>waves-effect<?php } ?>" style="width:100%;padding: 25px;padding-bottom:0!important;padding-left: 30px!important;padding-right:20px!important" <?php if(count($_DATA->reminders) !== 0) {?>onclick="$('#sidenav__todoList').sidenav('open')"<?php } ?>><?php if(count($_DATA->reminders) !== 0) {?><i class="material-icons right" style="line-height: 40px;">chevron_right</i><?php } ?><h5 style="margin-top: 3px"><b>Reminders</b></h5></div>
    <div class="card-content" style="padding-top: 0!important">
      <?php foreach (array_slice($_DATA->reminders, 0, 10, true) as $item) {
        ?>
          <p style="line-height: 30px">
          <label>
            <input type="checkbox" oninput="if(confirm('Delete?')===true){this.disabled=true;loadURL('./user/todo/delete.php?id=<?=$item->id;?>','GET',function(){ setTimeout(function() {getHashPage('hide')}, 200) })}else{this.checked=false}"/>
            <span><?=$item->name;?></span>
          </label>
        </p>
        <?php
      }
      if(count($_DATA->reminders) == 0) {
        ?>
        <center>
    ...
    <?php foreach ($_DATA->listNames as $item) { if(!isEven($item->id)) {?>
        ...
        <?php foreach ($_DATA->listItems as $item1) { if($item1->listId == $item->id) { ?>
          <p style="line-height: 30px">
          <label>
            <input type="checkbox" oninput="if(confirm('Delete?')===true){this.disabled=true;loadURL('./user/items/delete.php?id=<?=$item1->id;?>','GET',function(){ setTimeout(function() {getHashPage('hide')}, 200) })}else{this.checked=false}"/>
            <span><?=$item1->name;?></span>
          </label>
        </p>
        <?php } }?>
...
<div id="addItem1" class="modal">
  <div class="modal-content">
    <form action="./user/lists/addItem.php" id="addItemForm" method="POST">
      <h5>Create new</h5>
      ...
      <button class="w-full-sm btn blue-grey lighten-4 waves-effect black-text btn-round modal-close sidenav-close" style="min-width: 100px;border-radius: 15px!important">
...
$(document).ready(function() {
  M.Sidenav.init(document.querySelectorAll("#sidenav__shoppingList, #sidenav__todoList"), {
    edge: "right",
   ...
   <?php foreach ($_DATA->listNames as $list) {?>
    M.Sidenav.init(document.getElementById("sidenav__<?=$list->id;?>"), {
    edge: "right",
    onOpenStart() {document.querySelector("meta[name='theme-color']").setAttribute("content", (document.documentElement.classList.contains("_darkTheme" ? "#101010" : "#7a7a7a")));},
    onCloseEnd: navColor
  });
...
document.getElementById("addItemForm").addEventListener("submit", (event) => {
  sendData(event).then(function(data) {
    M.toast({text:data});
    getHashPage("hide");
    M.Sidenav.getInstance(document.querySelectorAll('.sidenav:not(#slide-out)')).close()
    M.Modal.getInstance(document.querySelectorAll('.modal')).close()
  });
    // M.Sidenav.getInstance(document.querySelectorAll('.sidenav:not(#slide-out)')).close();
    // M.Modal.getInstance(document.querySelectorAll('.modal')).close()
}
