// import 'https://smartlist.ga/dashboard/js/lazyLoad.js';
...
function notifyMe() {
  if (!("Notification" in window)) {
      if(localStorage.getItem('n_popup') !== 1) {
      localStorage.setItem("n_popu[", 1)
        M.toast({
...
window.addEventListener('popstate', function(event) {
  ...
  if ($(".tabcontent").is(":visible")) {
    if (!$("#item_popup").is(":visible") && !$("#modal1").is(":visible") && !$(".modal").is(":visible")) {
      $('#slide-out').scrollTop(0);
      sm_page('News');
