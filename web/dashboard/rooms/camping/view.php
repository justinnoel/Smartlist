...
<td onclick="campingSort(1)">Quantity</td>
      ...
      {
        echo "<tr data-date='".($row['date'])."' class='".($row['login_id'] !== $_SESSION['id'] ? "sync_tr" : "")."' id='campingtr_" . $row['id'] . "' onclick='item(this, ".($row['star'] == 1 ? 1 : 0).", ".json_encode(decrypt($row['price'])).", \"camping\")' ".($row['star'] == 1 ? "style='border-left: 3px solid #f57f17'" : "").">
