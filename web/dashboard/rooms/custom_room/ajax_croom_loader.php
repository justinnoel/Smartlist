...
try {
  ...
  foreach ($users as $row) {
      echo "<tr data-id='".$row['id']."' class='".($row['login_id'] != $_SESSION['id'] ? "sync" : "")."' id='croomtr_".$row['id']."' onclick='item(this, ".($row['star'] == 1 ? 1 : 0).", ".json_encode(($row['label'])).", \"custom_room\")' ".($row['star'] == 1 ? "style='border-left: 3px solid #f57f17'" : "").">";
