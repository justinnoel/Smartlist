...
<html>
  <head>
    ...
    <base href="https://smartlist.ga/dashboard/beta">
  ...
  <body>
    ...
    <nav style="background: var(--navbar-color) !important">
      <ul>
        <li>
    ...
    <div id="avatarChange" class="modal">
      <div class="modal-content">
        <center>
          <img class="circle" src="<?=$user['avatar'];?>" class="center" style="width: 100px" id="src">
        </center>
    ...
    <div id="feedbackBeta" class="modal" style="height: auto;border-radius: 10px">
      <img src="https://cdn.dribbble.com/users/389484/screenshots/14612449/media/70ae3a03b150ccd663489d0180eaaef5.png?compress=1&resize=1200x900" style="width: 100%;height: 300px;object-fit: cover">
      <div class="modal-content">
    ...
    <ul id="slide-out" class="sidenav sidenav-fixed">
      <li><div class="user-view">
        ...
        <a href="#avatarChange" class="modal-trigger"><img class="circle" src="<?=$user['avatar'];?>"></a>
        <a href="#name"><span class="white-text name"><?=$user['name'];?></span></a>
        <a href="javascript:void(0)"><span class="white-text email"><?=$user['email'];?></span></a>
        </div></li>
      <li class="sidenav-close links"><a oncontextmenu="return false" onclick="sidenav_highlight(this)" class="waves-effect" href="#/dashboard"><i class="material-icons">dashboard</i>Dashboard</a></li>
      ...
      <li class="sidenav-close links"><a oncontextmenu="return false" onclick="sidenav_highlight(this)" class="waves-effect" href="#/my-finances"><i class="material-icons">money</i> My Finances</a></li>
      ...
      <li class="sidenav-close links"><a class="sidenav-close waves-effect" href="#/rooms/laundry" ><i class="material-icons-round left">local_laundry_service</i>Laundry room</a></li>
      <li class="sidenav-close"><div class="divider"></div></li>
      <li class="sidenav-close"><a class="subheader">Custom Rooms</a></li>
      <?php try {
  $dbh = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
  $sql = "SELECT * FROM roomnames WHERE login_id=" . $_SESSION['id'] . " OR login_id= " . json_encode(decrypt($_SESSION['syncid']));
  $users = $dbh->query($sql);
  foreach ($users as $row) {
    print "<li class='links'><a oncontextmenu='croom_rclick(".$row['id'].");return false' class=\"waves-effect sidenav-close\" href='#/rooms/".$row['id']."'><i class=\"material-icons-round left\">" . $row['qty'] . "</i>" . $row["name"] . "</a></li>\n";
  }
  $dbh = null;
  ...
  echo "Error is: " . $e->getmessage();
    ...
    <div id="edit_sidenav" class="sidenav" style="width: 100%;z-index: 9999999999999999999999999">
    ...
    <div id="item_sidenav" style="width: 100%;overflow-x: hidden">
      <div class="container">
        <nav class="blue-grey darken-4" style="position: fixed;top: 0;left: 0;z-index: 999999999999">
          <ul style="padding: 0 !important;margin: 0 !important">
            <li>
              <a href="javascript:void(0)" class="sidenav-close" style="color: white !important;line-height: 60px">
                <i class="material-icons left" style="line-height: 60px;color: white !important">arrow_back</i>Details</a>
            </li>
          </ul>
          <ul class="right">
    ...
    <div id="bmmodal" class="bottom-sheet modal modal-fixed-footer">
      <form onsubmit="bm_add();M.toast({html: 'Added data successfully!'});return false;" method="post" name="form1"> <input type="hidden" name="name" value="09/20/2021" readonly>
        ...
        <div class="modal-content">
          <h4>Add a point</h4>
          <h5>Quick Add</h5>
          <div class="chip_suggestions">
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$5</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$10</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$15</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$20</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$25</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$30</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$50</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$100</div>
            <div class="chip waves-effect" onclick="document.getElementById('bm_amount').value=this.innerText.replace('$', '');bm_add();M.toast({html: 'Added data successfully!'});">$150</div>
          </div>
          ...
          <p style="margin-top: 20px"><i class="material-icons-round left">verified</i> We take your privacy very
            seriously, and we will never sell your data. <a href="https://support.smartlist.ga/docs/#/privacy-policy" class="blue-text">Read our Privacy Statement</a></p>
          <br>
        </div>
        <div class="modal-footer">
    ...
    <div class="fixed-action-btn">
      <a class="btn-floatiang btn-large waves-effect waves-light" id="fab">
      ...
      <ul>
        ...
        <li data-position="left" data-tooltip="Shopping List" class="tooltipped"><a class="fabBTN btn-floating"><i class="material-icons">receipt_long</i></a></li>
  ...
  echo "Error is: " . $e->getmessage();
    ...
    <script>
      ...
      function getHashPage(e = "") {
        ...
        document.querySelector('meta[name="theme-color"]').setAttribute('content',  '#<?=$theme_top?>');
        if($(".modal").is(":visible")) {
        ...
        switch(window.location.hash.replace("#", "")) {
          ...
          case "/settings": url = "./user/settings/index.php"; document.querySelector('meta[name="theme-color"]').setAttribute('content',  (document.documentElement.classList.contains("_darkTheme") ? "#101010": "#eee")); break
  ...
  try {
    ...
    echo "Error is: " . $e->getmessage();
      ...
      $(document).ready(() => {
        $('#item_sidenav, #edit_sidenav').sidenav({
          edge: "right"
        })
      })
