...
items.forEach((item,key) => {
  ...
  main.innerHTML += `<div class="carousel-item ${color} white-text darken-3" style="padding-top: 25vh" onclick="${(item.id ? "$('#ajaxLoader').load('https://smartlist.ga/dashboard/rooms/grocerylist/delete.php?id="+item.id+"}');" : "")} setTimeout(()=>\$('.carousel.carousel-slider').carousel('next'), 500);done();setTimeout(()=>this.classList.add('done'),1700);">
<h1>${item.name}</h1>
<h2>${item.room}</h2>
